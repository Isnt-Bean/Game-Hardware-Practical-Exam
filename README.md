...
## Part 1
### Section 1
<img width="1076" height="712" alt="image" src="https://github.com/user-attachments/assets/1aa9e90a-dc56-48f9-8304-0d0b627ebd5c" />
...
- The two buttons on the right signify the ability to punch with the left and right hand, with a DC motor as tactile cue for when you punch. These are hooked up to the potentiometer which changes the power of the attack based on if the player is blocking attacks or not. This makes the LEDs dimmer and the DC motor spin less.
- The potentiometer in the middle of the board signifies the block ability, with a lower powered LED and DC motor to let the player know the block is active. This is hooked up to the attack buttons and the DC motor by extention. This uses player input to determine if the attacks are being blocked or not. This is also the continuous input as the game must know if you are blocking at all times.
- The difference from the sketch and the final design was the removal of the attack types and the forward and backwards movement options. This was done to conserve space on the breadboard and for streamlined controls, allowing for an easier experience using it as a theoretical controller. The controller is set up the way it is because it is the standard layout for most modern controllers with movement on the left and the right used for all other inputs.
